...
class ChatService {
  final DatabaseReference _chatsRef = FirebaseDatabase.instance.ref().child(AppConstants.chats);
  final DatabaseReference _messagesRef = FirebaseDatabase.instance.ref().child(AppConstants.messages);
  ...
  Future<Map<Chat, Message?>> getAllChatsWithLastMessages() async {
    DataSnapshot chatsSnapshot = await _chatsRef.get();
    Map<Chat, Message?> chatsWithMessages = {};
    ...
    for (var chatSnapshot in chatsSnapshot.children) {
      ...
      Chat chat = Chat.fromMap(chatData, chatId);
      
      // Get last message
      DataSnapshot lastMessageSnapshot = await _messagesRef
          .child(chatId)
          .orderByChild(AppConstants.timeStamp)
          .limitToLast(1)
          .get();
      ...
      Message? lastMessage;
      if (lastMessageSnapshot.exists && lastMessageSnapshot.children.isNotEmpty) {
  ...
  Future<List<Map<String, dynamic>>> getUserChatsWithLastMessages(String userId) async {
    // Get all chats
    DataSnapshot chatsSnapshot = await _chatsRef.get();
    ...
    for (var chatSnapshot in chatsSnapshot.children) {
      ...
      List<String> otherMembers = members.where((member) => member != userId).toList();
  ...
  Future<Chat?> getChat(String chatId) async {
    ...
    if (snapshot.exists) {
      return Chat.fromMap(snapshot.value as Map<dynamic, dynamic>, chatId);
    }
