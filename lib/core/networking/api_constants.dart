abstract class ApiConstants {
  static const String baseUrl =
      'https://0cc05de28b9d4733f07c8f292cf10559.serveo.net/saturn/';
  static const String signUp = 'auth/signup.php';
  static const String signIn = 'auth/login.php';
  static const String upload = '${ApiConstants.baseUrl}upload/';
  static const String getCategorieItems = 'view/home.php';
  static const String addLike = 'add/like.php';
  static const String likedPosts = 'view/post.php';

  // https://89a0f66ba7898c6f51711d4d284fc4b4.serveo.net/saturn/add/like.php
  //https://4e2e6b7e6b97ac0bba414b3f87ee0e63.serveo.net/saturn/view/home.php

  //  https://ed0d-197-53-85-46.ngrok-free.app/saturn/auth/signup.php
}
